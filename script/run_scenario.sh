#!/bin/bash
cd $(dirname $0)
if [ ! -a scenariorunner2.class ] || [ scenariorunner2.java -nt scenariorunner2.class ] ; then
	if ! javac scenariorunner2.java ; then
		exit 1
	fi
fi
usage() {
...
if [ -z $FILE ] ; then
	...
	if [ $FORK -eq 1 ] ; then
		echo "Forking!"
		java scenariorunner2 $FILE $CONTROLLER_IP &
	else
		echo "Do not fork!"
		java scenariorunner2 $FILE $CONTROLLER_IP
	fi
